...
#endif
#define vfe_dev_err(x,arg...) printk("[CSI_ERR][GC0308]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[CSI][GC0308]"x,##arg)
    ...
    unsigned char *value) //!!!!be careful of the para type!!!
{
  ...
  vfe_dev_dbg("sensor read  reg=%02X,value=%02X\n",reg,*value);
  return ret;
}
    ...
    unsigned char value)
{
  ...
  if(cnt>0)
  	vfe_dev_dbg("sensor write retry=%d when reg=%02X,value=%02X\n",cnt,reg,value);
